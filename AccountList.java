...
public class AccountList {
	...
	public static void getAccountFromText() { // 파일에 저장되어 있는 Account객체 정보 가져오기
		try {
			...
			String income = bufReader.readLine(); // 수입 분류 정보 가져오기
			...
			String outcome = bufReader.readLine(); // 지출 분류 정보 가져오기
			...
			String line = "";
			while ((line = bufReader.readLine()) != null) { // 정보 가져와 Account생성 후 list에 삽입
				String[] list = line.split("/");
				AccountList.list.add(new Account(LocalDate.parse(list[0]), Integer.parseInt(list[1]), list[2], list[3],
	...
	public static void inputAccountToText() { // 시스템 종료시 데이터 모두 파일에 쓰기
		try {
			...
			while (iterator.hasNext()) {
				...
				String tmp = act.getPreparationDate().toString() + "/" + act.getMoney() + "/" + act.getType() + "/"
						+ act.getCategory() + "/" + act.getBalance() + "\n";
	...
	public static void changeBalance(Account act) { // 잔액이 바뀌면 바뀐 잔액 뒤부터 잔액
		Iterator<Account> iterator = list.iterator();
		boolean check = false;
		int preBalance = 0;
		while (iterator.hasNext()) {
	...
	public static void changeType(Account act, int money) { // 수입or지출을 바꾸는 함수
		Iterator<Account> iterator = list.iterator();
		boolean check = false;
		while (iterator.hasNext()) {
			Account tmp = iterator.next();
	...
	public static void changeMoney(Account act, int money) { // 금액 바꾸는 함수
		Iterator<Account> iterator = list.iterator();
		boolean check = false;
		while (iterator.hasNext()) {
			Account tmp = iterator.next();
	...
	public static ArrayList<String> getYearList() { // 가지고 있는 Account의 년도 가져오기
		Iterator<Account> iterator = list.iterator();
