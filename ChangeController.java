...
public class ChangeController implements Initializable {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		preparationDate.setCellValueFactory(
				cellData -> new SimpleStringProperty(cellData.getValue().getPreparationDate().toString()));
		type.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getType()));
		money.setCellValueFactory(
				cellData -> new SimpleStringProperty(Integer.toString(cellData.getValue().getMoney())));
		category.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getCategory()));
		balance.setCellValueFactory(
				cellData -> new SimpleStringProperty(Integer.toString(cellData.getValue().getBalance())));
		accountTableView.setItems(FXCollections.observableList(
				AccountList.getAccount(Integer.toString(date.getYear()), Integer.toString(date.getMonthValue()))));
	...
	public void check(ActionEvent event) {
		if (yearBox.getValue() != null && monthBox.getValue() != null)
			accountTableView.setItems(
					FXCollections.observableList(AccountList.getAccount(yearBox.getValue(), monthBox.getValue())));
	}

	@FXML
	public void change() throws Exception {
		try {
			if (chType.getValue() != null && chCategory.getValue() != null && chDate.getValue() != null
					&& chMoney.getText() != null) {
				...
				if (act.getPreparationDate() != chDate.getValue()) {
					...
					Account tmp = new Account(chDate.getValue(), Integer.parseInt(chMoney.getText()), chType.getValue(),
							chCategory.getValue(), 0);
